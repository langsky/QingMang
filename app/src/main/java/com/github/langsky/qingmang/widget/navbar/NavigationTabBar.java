...
package com.github.langsky.qingmang.widget.navbar;
...
public class NavigationTabBar extends View implements ViewPager.OnPageChangeListener {
    ...
    protected final ValueAnimator animator = new ValueAnimator();
    protected final ResizeInterpolator resizeInterpolator = new ResizeInterpolator();
    ...
    protected int badgeBgColor = AUTO_COLOR;
    ...
    public NavigationTabBar(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        try {
            ...
            setBgColor(
                    typedArray.getColor(
                            R.styleable.NavigationTabBar_bgColor, DEFAULT_BG_COLOR
                    )
            );
            ...
            // Init animator
            animator.setFloatValues(MIN_FRACTION, MAX_FRACTION);
            ...
            if (isInEditMode()) {
                ...
                try {
                    ...
                    if (previewColors == null)
                        previewColors =
                                typedArray.getResources().getStringArray(R.array.colors);

                    for (String previewColor : previewColors)
                        models.add(new Model.Builder(null, Color.parseColor(previewColor)).build());
                    requestLayout();
                }
            }
        } finally {
    ...
    public void setModels(final List<Model> models) {
        //Set setUiData listeners to badge model animation
    ...
    public void setViewPager(final ViewPager viewPager) {
        ...
        if (viewPager.equals(this.viewPager)) return;
        if (this.viewPager != null) //noinspection deprecation
    ...
    public void setViewPager(final ViewPager viewPager, int index) {
        ...
        mIndex = index;
        if (mIsViewPagerMode) this.viewPager.setCurrentItem(index, true);
        postInvalidate();
    }
    ...
    public void setModelIndex(final int modelIndex, final boolean isForce) {
        ...
        index = Math.max(0, Math.min(index, models.size() - 1));
        ...
        if (mIsViewPagerMode) {
            ...
            viewPager.setCurrentItem(index, !force);
        }
        ...
        // If it force, so setUiData immediately, else animate
        ...
        if (force) {
            ...
            if (mOnTabBarSelectedIndexListener != null)
                mOnTabBarSelectedIndexListener.onStartTabSelected(models.get(mIndex), mIndex);

            // Force onPageScrolled listener and refresh VP
            if (mIsViewPagerMode) {
                if (!viewPager.isFakeDragging()) viewPager.beginFakeDrag();
                if (viewPager.isFakeDragging()) viewPager.fakeDragBy(0.0F);
                if (viewPager.isFakeDragging()) viewPager.endFakeDrag();
            } else {
                if (mOnTabBarSelectedIndexListener != null)
                    mOnTabBarSelectedIndexListener.onEndTabSelected(models.get(mIndex), mIndex);
            }
        } else animator.start();
    }
    ...
    public boolean onTouchEvent(final MotionEvent event) {
        ...
        if (scrollState != ViewPager.SCROLL_STATE_IDLE) return true;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsPointerActionDown) {
                    if (mIsHorizontalOrientation)
                        viewPager.setCurrentItem((int) (event.getX() / mModelSize), true);
                    else
                        viewPager.setCurrentItem((int) (event.getY() / mModelSize), true);
                    break;
                }
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        if (models.isEmpty() || width == 0 || height == 0) return;
        ...
        if (width > height) {
            ...
            // Get model size
            mModelSize = (float) width / (float) models.size();
            ...
            mModelSize = (float) height / (float) models.size();
        ...
        if (isInEditMode() || !mIsViewPagerMode) {
            ...
            if (isInEditMode()) {
                mIndex = new Random().nextInt(models.size());
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        for (int i = 0; i < models.size(); i++) {
            mPaint.setColor(models.get(i).getColor());
        ...
        for (int i = 0; i < models.size(); i++) {
            ...
            final float interpolation =
                    resizeInterpolator.getResizeInterpolation(mFraction, true);
            final float lastInterpolation =
                    resizeInterpolator.getResizeInterpolation(mFraction, false);
        ...
        for (int i = 0; i < models.size(); i++) {
            ...
            else mBadgePaint.setColor(badgeBgColor == AUTO_COLOR ? mActiveColor : badgeBgColor);
    ...
    public void onPageScrolled(int position, float positionOffset, final int positionOffsetPixels) {
        ...
        if (!animator.isRunning() && mIsSetIndexFromTabBar) {
    ...
    public void onPageScrollStateChanged(final int state) {
        ...
        if (state == ViewPager.SCROLL_STATE_IDLE) {
            if (onPageChangeListener != null) onPageChangeListener.onPageSelected(mIndex);
    ...
    public static class Model {
        ...
        Model(final Builder builder) {
            ...
            mBadgeAnimator.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(final Animator animation) {
                    ...
                    // Detect whether we just setUiData text and don`t reset show state
                ...
                public void onAnimationRepeat(final Animator animation) {
                    // Change title when we setUiData and don`t see the title
        ...
        // If your badge is visible on screen, so you can setUiData title with animation
